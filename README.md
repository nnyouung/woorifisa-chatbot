...
- **Cause:** ::before 가상 요소가 절대 위치(position: absolute)로 생성되며 채팅 UI 요소들과 동일한 스택 컨텍스트에서 렌더링됨

- **Solution:** 채팅 UI 자식 요소에 z-index: 1을 지정해 배경 레이어보다 위에 오도록 조정
  ```
  .chat-app > * {
    z-index: 1;
  }
